      ...
      >
        <div className="max-w-4xl mx-auto px-6">
          ...
          <header className="mb-16 text-center space-y-8">
            ...
            >
              <span
                className={`text-sky-600 dark:text-sky-400 font-black uppercase tracking-[0.2em] flex items-center gap-1.5 ${isAr ? "text-[12px]" : "text-[10px]"}`}
              >
                <FaHashtag /> {post.category || "Engineering"}
              </span>
              <div className="w-[1px] h-3 bg-slate-200 dark:bg-slate-800" />
              <span
                className={`text-slate-400 dark:text-slate-500 font-bold uppercase tracking-[0.2em] flex items-center gap-1.5 ${isAr ? "text-[12px]" : "text-[10px]"}`}
              >
            ...
            <h1
              className={`font-black text-slate-900 dark:text-white leading-[1.15] tracking-tight max-w-3xl mx-auto text-balance ${isAr ? "text-4xl lg:text-6xl" : "text-5xl lg:text-7xl"}`}
            >
              {post.title}
            </h1>
          </header>
          ...
          <footer className="mt-24 p-10 bg-slate-50 dark:bg-slate-900 rounded-[3rem] border border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-8 transition-all">
            <div
              className={`flex items-center gap-6 text-center md:text-start ${isAr ? "md:flex-row-reverse md:text-right" : "md:flex-row"}`}
            >
              <div className="w-20 h-20 rounded-[2rem] overflow-hidden border-4 border-white dark:border-slate-800 shadow-xl rotate-3 bg-white dark:bg-slate-800 flex items-center justify-center transition-colors">
                {post.author?.profileImage ? (
                  <img
                    src={`${IMAGE_BASE}${post.author.profileImage}`}
              ...
              <div className="flex flex-col">
                <p
                  className={`font-black text-sky-600 dark:text-sky-400 uppercase tracking-[0.3em] mb-1 ${isAr ? "text-[12px]" : "text-[10px]"}`}
                >
                  {t("writtenBy")}
                </p>
                <p
                  className={`font-black text-slate-900 dark:text-white leading-tight ${isAr ? "text-3xl" : "text-2xl"}`}
                >
                  {post.author?.name || "Sameh Dheir"}
                </p>
                <p
                  className={`font-bold text-slate-400 dark:text-slate-500 italic mt-1 ${isAr ? "text-[15px]" : "text-sm"}`}
                >
                  {post.author?.title}
                </p>
              </div>
            ...
            <button
              className={`px-8 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white font-black rounded-2xl hover:bg-sky-600 dark:hover:bg-sky-500 hover:text-white hover:border-sky-600 transition-all shadow-sm ${isAr ? "text-[15px]" : "text-sm"}`}
            >
              {t("followInsights")}
            </button>
          </footer>
