...
export default function DashboardPage() {
  ...
  if (postsLoading || projectsLoading || skillsLoading) {
    return (
      <div className="flex h-[80vh] items-center justify-center">
        <Loader2 className="animate-spin text-indigo-600 dark:text-sky-500" size={48} />
      </div>
    );
  }
  ...
  const stats = [
    ...
    },
    {
      title: "Active Skills",
      value: totalSkills,
      icon: <Code2 className="text-emerald-600 dark:text-emerald-400" size={24} />,
    ...
    },
    {
      title: "Blog Articles",
      value: totalPosts,
      icon: <Newspaper className="text-indigo-600 dark:text-indigo-400" size={24} />,
    ...
    <div className="p-8 max-w-7xl mx-auto transition-colors duration-500">
      <header className="mb-12 flex justify-between items-end">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">
            Welcome Back, <span className="text-indigo-600 dark:text-sky-500">Sameh!</span>
          </h1>
        ...
        <div className="text-right hidden md:block">
           <p className="text-xs font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">Last Update</p>
           <p className="text-sm font-black text-slate-700 dark:text-slate-300">{new Date().toLocaleDateString()}</p>
        </div>
      </header>
        ...
        {stats.map((stat, idx) => (
          ...
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <a href={stat.link} className="text-xs font-bold text-slate-400 dark:text-slate-500 flex items-center gap-1 group-hover:text-indigo-500 dark:group-hover:text-sky-400 transition-colors">
                Manage <ArrowUpRight size={14} />
              </a>
            </div>
            <h3 className="text-slate-500 dark:text-slate-400 font-bold text-sm uppercase tracking-wider mb-1">{stat.title}</h3>
            <div className="flex items-baseline gap-2">
              <p className="text-4xl font-black text-slate-900 dark:text-white">{stat.value}</p>
              <span className="text-xs font-bold text-slate-400 dark:text-slate-500">{stat.trend}</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-center transition-all">
          <h2 className="text-xl font-black text-slate-900 dark:text-white mb-6">Content Distribution</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-5 bg-emerald-50 dark:bg-emerald-900/10 rounded-3xl border border-emerald-100 dark:border-emerald-900/20">
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-emerald-900 dark:text-emerald-400">Published Content</span>
              </div>
              <span className="text-3xl font-black text-emerald-600 dark:text-emerald-500">{publishedPosts}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-5 bg-amber-50 dark:bg-amber-900/10 rounded-3xl border border-amber-100 dark:border-amber-900/20">
              <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-amber-900 dark:text-amber-400">Pending Drafts</span>
              </div>
              <span className="text-3xl font-black text-amber-600 dark:text-amber-500">{draftPosts}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 dark:bg-indigo-950 p-8 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden group border border-transparent dark:border-indigo-900/50 transition-all">
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div>
              ...
              <h2 className="text-2xl font-black mb-4 group-hover:text-indigo-400 dark:group-hover:text-sky-300 transition-colors">Architecture Matters.</h2>
            ...
            <a href="dashboard/projects" className="mt-8 bg-indigo-600 dark:bg-indigo-700 p-3 text-center border-0 text-white w-full py-4 rounded-2xl font-bold hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-all shadow-lg active:scale-95 border border-indigo-500">
              Add New Project
            </a>
          </div>
