...
const IMAGE_BASE = process.env.NEXT_PUBLIC_IMAGE_BASE_URL;
  ...
  return (
    <section
      id="blog"
      className="py-24 bg-transparent overflow-hidden text-start transition-colors duration-500"
    >
        ...
        >
          <div className="space-y-4">
            ...
            <h2 className="text-5xl lg:text-7xl font-black text-slate-900 dark:text-white tracking-tighter">
              {t("title")}{" "}
              <span className="text-red-500  decoration-sky-100 dark:decoration-sky-900/30">
                {t("titleAccent")}
              </span>
            </h2>
          </div>
          <Link
            href={`/${locale}/blog`}
            className="group text-slate-900 dark:text-slate-300 font-black flex items-center gap-2 hover:text-sky-600 dark:hover:text-sky-400 transition-colors rtl:flex-row-reverse"
          >
            {t("viewArchive")}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-[400px] bg-slate-100 dark:bg-slate-800 animate-pulse rounded-[2.5rem]"
              />
            ))}
          </div>
        ) : (
            ...
            {featuredPosts.map((post: any) => (
              ...
              >
                ...
                <div className="flex flex-col flex-1">
                  ...
                  <h3 className="text-xl font-black text-slate-900 dark:text-white leading-tight mb-4 group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">
                    {post.title}
                  </h3>
                  ...
                  <Link
                    ...
                    className="mt-auto inline-flex items-center gap-2 text-slate-900 dark:text-white font-black text-sm group/btn rtl:flex-row-reverse"
                  >
                    {t("continueReading")}
                    <motion.span
                      animate={{ x: locale === "en" ? [0, 5, 0] : [0, -5, 0] }}
