        ...
        >
          <div className="space-y-4">
            ...
            <h2 className="text-5xl lg:text-7xl font-black text-slate-900 tracking-tighter">
              Latest{" "}
              <span className="text-red-500 underline decoration-sky-100 underline-offset-[10px]">
                Insights.
              </span>
            </h2>
          </div>
          <Link
            href="/blog"
            className="group text-slate-900 font-black flex items-center gap-2 hover:text-sky-600 transition-colors"
          >
            View Archive{" "}
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-[400px] bg-slate-100 animate-pulse rounded-[2.5rem]"
              />
            ))}
          </div>
        ) : (
              ...
              featuredPosts.map((post: any) => (
                ...
                >
                  <div className="relative aspect-[16/11] rounded-3xl overflow-hidden mb-6 bg-slate-50">
                    ...
                    />
                    <div className="absolute top-4 left-4 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-xl text-[10px] font-black text-sky-600 uppercase flex items-center gap-1 shadow-sm">
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <h3 className="text-xl font-black text-slate-900 leading-tight mb-4 group-hover:text-sky-600 transition-colors">
                      {post.title}
                    </h3>
                    ...
                    <Link
                      href={`/blog/${post.slug}`}
                      className="mt-auto inline-flex items-center gap-2 text-slate-900 font-black text-sm group/btn"
                    >
                      Continue Reading
                      <motion.span
                        animate={{ x: [0, 5, 0] }}
