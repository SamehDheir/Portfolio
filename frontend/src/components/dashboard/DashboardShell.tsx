      ...
      `}>
        <div className="p-8 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-600 dark:bg-sky-600 rounded-xl flex items-center justify-center text-white shadow-lg font-black text-xl">S</div>
            <div>
              <h1 className="text-lg font-black text-slate-900 dark:text-white leading-tight transition-colors">Sameh.Dev</h1>
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Admin Engine</span>
            </div>
          </div>
        ...
        <nav className="flex-1 mt-4 px-4 space-y-1 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {item.icon} {item.name}
                {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-600 dark:bg-sky-500 animate-pulse" />}
              </Link>
            );
          })}
        </nav>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800">
          <Link href="/dashboard/profile" className="flex items-center gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 mb-2 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all group overflow-hidden">
            <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white dark:border-slate-700 shadow-sm flex-shrink-0">
              {user?.profileImage ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-indigo-600 dark:text-sky-400 bg-white dark:bg-slate-700"><UserCircle size={24} /></div>
              )}
            </div>
            <div className="overflow-hidden">
              <p className="text-sm font-black text-slate-900 dark:text-white truncate">{user?.name || "Sameh Dheir"}</p>
              <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase truncate">{user?.title || "Senior Developer"}</p>
            </div>
          </Link>

          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-red-500 dark:text-red-400 font-bold hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all">
